...
class MobileDropdownWalker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if ($has_children) {
            ...
            // Link do strony nadrzędnej – pierwsze kliknięcie otwiera submenu, drugie nawiguje.
            $output .= '<a href="' . esc_attr($item->url) . '" @click.stop="if(!open){ $event.preventDefault(); open = true; }" class="flex-grow py-1 pr-4">';
            $output .= esc_html($item->title);
            $output .= '</a>';
            
            // Osobny przycisk ze strzałką, który rozwija/zwija submenu.
            $output .= '<button @click.stop="open = !open" class="p-2 -mr-2 shrink-0">';
