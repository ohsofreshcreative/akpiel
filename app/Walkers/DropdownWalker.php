...
class DropdownWalker extends Walker_Nav_Menu
{
    ...
    {
        $classes = 'absolute z-10 mt-2 w-max origin-top-right bg-white border-primary focus:outline-none right-0';
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if ($depth === 0 && $has_children) {
            ...
            $output .= '<a href="' . esc_attr($item->url) . '"' . $target . ' class="inline-flex items-center gap-x-1 text-sm font-medium">';
        ...
        // Case 2: Pozostałe elementy (zwykłe linki na górze lub linki wewnątrz dropdownu)
            ...
            $link_classes = '';
            if ($depth > 0) {
                $link_classes = 'block rounded-xl px-6 py-4 text-sm  !font-bold  !text-secondary hover:bg-primary hover:!text-white';
            } else {
                $link_classes = 'text-sm font-medium hover:text-indigo-600';
            }

            $target = !empty($item->target) ? ' target="' . esc_attr($item->target) . '"' : '';
            $output .= '<a href="' . esc_attr($item->url) . '"' . $target . ' class="' . esc_attr($link_classes) . '">';
            $output .= esc_html($item->title);
            $output .= '</a>';
        }
    }
